...
wie Hifiberry DAC+ eingesetzt werden. Die Audioausgabe kann daher nur über HDMI
...
Vielmehr ist eine intuitive Bedieneroberfläche erforderlich. Auch in
\Bezeichnung\ kann diesbezüglich noch viel getan werden, das will ich gar nicht
abstreiten. Mehrere Punkte dazu sind in Kapitel \ref{sect:Erweiterungen}
aufgeführt.
...
kann derzeit noch nicht unterscheiden, ob es sich dabei um eine abspielbare
Mediendatei oder um einen anderen Dateityp (\zB eine Textdatei) handelt.\\
...
unsichtbar) und 255 (vollständig undurchsichtig) liegen kann. Grundsätzlich
...
Der Standardwert von 210 für die Transparenz lässt den Desktop und \Bezeichnung\
jedoch leicht durchscheinen, so dass eine Bedienung noch möglich ist.
...
\midrule[2pt]
\Code{matchbox virtual keyboard} & aktuell(?) & LGPL v... & \url{https://github.com/mwilliams03/matchbox-keyboard.git}\\
%\hline
\bottomrule[2pt]
\end{tabular}
\vspace{0.5cm}
...
werden können.\\
Die Software \Code{matchbox virtual keyboard} ist nur dann notwendig, wenn keine
...
\textit{wheezy} vom 05.05.2015, Imagedatei \filenam{2015-05-05-raspbian-wheezy.img}.
...
sich befinden. An den beschrieben Punkten hatte ich als grimmiger Python-Noob
selbst meine Probleme!\\
...
\item \filenam{matchbox virtual keyboard} installieren
\end{itemize}
...
Interaktion mit dem \RPi\ kann im späteren Verlauf der Inbetriebnahme auch über
\Code{ssh} erfolgen.
\end{bclogo}
...
auf die SD-Karte aufgespielt wurde das allererste, automatisch startende
...
Terminalfenster bitte folgendes Kommando ein:\\
\Code{./bauwong.py}\\
...
\begin{figure}[h]
\centering
\includegraphics[width=0.67\textwidth]{startmenu.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.67\textwidth]{desktop.png}
...
\item \textbf{Icon \texit{Toggle Matchbox Keyboard} in der Schnellstartleiste erstellen}\\
      ...
      \begin{figure}[h]
      \centering
      \includegraphics[width=0.8\textwidth]{launchbar.png}
      \caption{Änderungen in der Datei \filenam{/home/pi/.config/lxpanel/LXDE-pi/panels/panel}}
      \label{fig:launchbar}
      \end{figure}
      Im Abschnitt \textit{Plugin\arrowright type=launchbar} müssen die markierten Zeilen aus Abbildung \ref{fig:launchbar} ergänzt werden.
